...
class ThirtyOneDaysApp(toga.App):
    ...
    def check_in_task(self, task):
        ...
        if not task.can_check_in_today:
            self.show_notification(f'任务：{task.name}，今天已经打过卡了。')
            return
    ...
    def show_notification(self, message):
        ...
        # 使用自定义延时函数关闭通知
        self.add_background_task(self.custom_delay_close_notification)
    
    def custom_delay_close_notification(self, widget=None, **kwargs):
        """使用自定义延时函数关闭通知"""
        # 使用循环创建延时
    ...
    def _close_notification_on_main_thread(self, widget=None, **kwargs):
        """在主线程中关闭通知"""
